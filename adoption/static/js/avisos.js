...
let contact_channels = {
    "select": [document.getElementById('contact-channels-1')],
    "inputs": [document.getElementById('contact-id-or-url-1')],
};
...
let pet_image_inputs = [document.getElementsByClassName('pet-image')[0]];
...
const add_contact_channel = () => {
    if (contact_channels_counter < max_contact_channels) {
        ...
        contact_channel_select.name = 'contact-channels-' + contact_channels_counter;
        contact_channel_select.id = 'contact-channels-' + contact_channels_counter;
        ...
        contact_channel_input.name = 'contact-id-or-url-' + contact_channels_counter;
        contact_channel_select.id = 'contact-id-or-url-' + contact_channels_counter;
...
const add_image_inputs = () => {
    if (pet_images_counter < max_pet_images) {
        ...
        image_upload_input.className = 'pet-image'
        image_upload_input.type = 'file';
        image_upload_input.accept = "image/*";
        image_upload_input.name = 'pet-image-' + pet_images_counter;
...
const verify_pet_delivery_date = () => {
    ...
    return pet_delivery_date_input.valueAsNumber >= (Date.now() - 3 * 60 * 60 * 1000) 
    ? "" 
