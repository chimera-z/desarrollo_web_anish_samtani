...
let region_select_ = document.getElementById('region');

const add_regions = () => {
    for (const region of region_comuna["regiones"]) {
        let region_option = document.createElement('option');
        region_option.value = region["nombre"];
        region_option.innerHTML = region["nombre"];
        region_select_.appendChild(region_option);
    }
};
  
const change_communes = () => {
    console.log('Changing to communes correspondent to the region.')
    commune_select.removeAttribute("disabled");

    let base_opt = commune_select.children[0];
    commune_select.innerHTML = "";
    commune_select.appendChild(base_opt)
    commune_select.value = "None";

    for (const region of region_comuna["regiones"]) {
        if (region["nombre"] === region_select_.value) {
            for (const commune of region["comunas"]) {
                let commune_option = document.createElement('option');
                commune_option.value = commune["nombre"];
                commune_option.innerHTML = commune["nombre"];
                commune_select.appendChild(commune_option);
            }
        } 
    }
};

region_select_.addEventListener('change', change_communes);
window.onload = () => {
    add_regions();
};